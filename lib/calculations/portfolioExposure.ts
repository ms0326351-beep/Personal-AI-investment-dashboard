 ...
 * Limits are per position. No reverse/invented edges, ETF look-through or news inference.
 */
export function calculatePortfolioExposure(
  portfolio: ExposurePortfolioSnapshot,
  dataset: ExposureDataset,
  options: {maxDepth?:number; maxPathsPerPosition?:number}={},
    ...
    function walk(entityIds:string[], path:ExposureRelationship[]) {
      ...
      for(const edge of outgoing) {
        ...
        if(edge.dimension) {
          ...
          const confirmed=!indirect && fullEvidence && edge.confidence!=='unknown' && ['FACT','REPORTED'].includes(claim) && edge.counterEvidenceIds.length===0 && edge.measurement && Date.parse(edge.measurement.asOf)<=at;
        ...
        // Asset-to-Asset edges would be ETF/ownership look-through, intentionally out of scope.
  ...
  function concentration(dimension:ExposureDimension,paths:ExposurePath[]):ExposureConcentration {
    const overlapping=dimension==='theme';
    ...
    for(const p of portfolio.positions) {
      const eligible=paths.filter(path=>path.positionId===p.id && path.nature==='DIRECT' && path.knowledge==='KNOWN_QUANTITATIVE' && path.measurements[0]?.measurement.basis==='positionAllocation');
  ...
  const dimensions=Object.fromEntries(EXPOSURE_DIMENSIONS.map(dimension=>{
    ...
    return [dimension,{paths,coverage:{status:covered.size===0?'UNKNOWN':!truncated && unknownPositionIds.length===0?'KNOWN':'PARTIAL',
      ...
      ...(['sector','industry','theme'].includes(dimension)?{concentration:concentration(dimension,paths)}:{})}];
  })) as PortfolioExposureSummary['dimensions'];
  return {portfolioId:portfolio.portfolioId,portfolioVersion:portfolio.version,datasetVersion:dataset.version,asOf:portfolio.asOf,baseCurrency:portfolio.baseCurrency,
