...
export default function RootLayout({children}:{children:React.ReactNode}){return <html lang="zh-Hant"><body><a className="skip-link" href="#main">跳至主要內容</a><Navigation/><div className="workspace"><header className="topbar"><span>投資工作台 <span className="breadcrumb">／ 每日情報</span></span><span className="mock-badge"><span className="status-dot"/>行情測試模式</span></header><main id="main">{children}</main><footer>拾光投資 · 市場資料測試 <span>行情可能延遲或使用模擬備援；新聞、人物、AI、持股與匯率仍為模擬，非投資建議。</span></footer></div></body></html>}
