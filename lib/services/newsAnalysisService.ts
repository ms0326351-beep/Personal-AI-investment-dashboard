...
import { selectNewsAnalysisInput, type NewsInputBasis } from '../utils/newsAnalysisInput';

export const NEWS_ANALYSIS_SCHEMA_VERSION='v2';
...
export function createNewsAnalysisService(options: {
  ...
  async function read(key:string,item:NewsItem,model:string,retry=false):Promise<CachedNewsAnalysis|null> {
    ...
    if (cached.status==='unavailable' && cached.market===null && typeof cached.unavailableReason==='string') {
      const retryAt=Date.parse(cached.retryAt ?? '') || Date.parse(cached.analyzedAt)+30000;
      if(retry && cached.errorCode!=='daily_limit' && cached.unavailableReason!=='今日 AI 分析次數已達上限' && now()>=retryAt) return null;
      return cached;
    }
    if (cached.status==='ok') {
      if(schemaVersion===NEWS_ANALYSIS_SCHEMA_VERSION && !isDeepMarketImpact(cached.market)) return null;
  ...
  async function create(item:NewsItem,key:string,model:string,inputBasis:NewsInputBasis,retry:boolean):Promise<CachedNewsAnalysis> {
    ...
    if(!acquired) {
      ...
      for(let attempt=0;attempt<20;attempt++) {
        ...
        const result=await read(key,item,model,retry); if(result) return result;
      }
    ...
    try {
      ...
      try {
        ...
        success=true;
        result={newsId:item.id,status:'ok',schemaVersion,modelVersion:model,market,disclaimer:NEWS_AI_DISCLAIMER,analyzedAt:new Date(now()).toISOString(),inputBasis};
      } catch(error) {
        const failure=classifyAnalysisError(error);
        console.warn('[news-ai] Analysis failed',JSON.stringify({newsId:item.id,code:failure.code,upstreamStatus:failure.httpStatus,fields:failure.fields}));
        result={...unavailable(item.id,model,failure.userMessage),errorCode:failure.code,retryAt:new Date(now()+failure.retryDelaySeconds*1000).toISOString()};
      }
  ...
  return {
    async getOrCreateAnalysis(item:NewsItem,holdings:Holding[],watchlist:string[],retry=false):Promise<NewsAIAnalysis> {
      try {
        if(item.origin!=='rss') return unavailableNewsAnalysis(item.id,'模擬備援新聞不進行 AI 分析');
        const input=selectNewsAnalysisInput(item);
        if(!input) return unavailableNewsAnalysis(item.id,'找不到可供分析的新聞內容');
        item=input.item;
        const model=(options.model ?? getNewsAIModel)();
        const key=buildCacheKey(item.id,schemaVersion,model);
        let pending=inFlight.get(key);
        if(!pending) {
          pending=create(item,key,model,input.basis,retry).catch(()=>unavailable(item.id,model,'分析快取或協調服務暫時異常，請稍後重試'));
